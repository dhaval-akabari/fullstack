    ...
    <!-- Page Heading -->
      ...
      <div class="card shadow mb-4">
          ...
          <div class="card-body">
            ...
            <button @click.prevent="save" type="button" class="btn btn-primary">Save Post</button>
          </div>
      </div>
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async save() {
            ...
            const res = await this.callApi('put', `/admin/blog/edit/${id}`, this.data);
            if(res.status === 201) {
        ...
        async getBlog() {
            ...
            const res = await this.callApi('get', `/admin/blog/${id}`, []);
            if(!res.data) {
                return this.$router.push('/notfound');
            }
            if(res.status === 200) {
                ...
                this.data.post_excerpt = res.data.post_excerpt;
                this.data.metaDescription = res.data.metaDescription;

                for(let c of res.data.category){
                    this.data.category_id.push(c.id);
                }

                for(let t of res.data.tag){
